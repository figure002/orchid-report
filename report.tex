...
A simple strategy was needed for maintaining this collection of reference images and their classification labels. And ideal solution would be a database where these images are stored along with their meta-data (i.e. taxonomic information required for training the neural networks). Such a database needs to be accessible via an application programming interface (API) which would allow for the implementation of custom image harvesters for easy retrieval of reference images and their meta-data. Secondly, such a database would need to be accessible from the internet so that images and meta-data could be maintained collaboratively via the internet.

The online image hosting service Flickr was ultimately chosen to fulfil this requirement. Every photo was uploaded to an account on Flickr, where each image was annotated with standardized tags in the formats \verb/genus:name/, \verb/section:name/, and \verb/species:name/. The correct genus, section, and species for each image was verified by specialists and referenced to the literature \citet{Cribb1998, Pridgeon1999, Frosch2012} and phylogenetic reconstructions based on molecular analyses \citep{Li2011, Chochai2012}. Since the reference images need to be processed on a local computer, a custom Python script utilising the Flickr API was implemented to mirror the reference images and meta-data on a local hard-drive, where images are stored in a directory hierarchy such that the directory names correspond to the image classes. Another Python script uses the directory hierarchy to create a local meta-data database (\ref{sec:meta-database}) for a reference image set. This local meta-data database makes it possible to query images from the reference set in downstream analyses (e.g. process photos from orchids of the genus \textit{Paphiopedilum}).

\subsection{Image feature extraction}

Raw pixel data is usually not appropriate for pattern recognition algorithms, so images need to be summarised to one or more numerical values. These values, or features, describe some aspect of the image content. Many feature extraction algorithms are generic and are not limited to specific image types. Such feature extraction algorithms can applied to entire images or to regions of interest (ROI) in images. Many feature extraction algorithms have been developed and are usually well described in literature. Some have been implemented as separate software libraries for usage in other software. Image recognition software that use some type of pattern recognition usually implement one or more feature extraction algorithms to extract features from images. With so many feature extraction algorithms in the wild it can be a cumbersome task to find the right algorithm for a specific image recognition problem, let alone having to implement such algorithms yourself. Currently no comprehensive open-source library of image feature extraction algorithms exists (to our knowledge), and one of our goals was to develop such a library. The development of a Python package called ImgPheno (\url{https://github.com/naturalis/imgpheno}) initialised and a few feature extraction methods have been implemented.
...
Most ANN libraries come with many parameters (e.g. training algorithms, activation functions, neural network topology, etc.), and since there is no way of saying which parameters work best with a specific problem, setting parameters becomes a matter of trial-and-error. To overcome this problem, a genetic algorithm was implemented for the training process. For this purpose a framework for evolving optimal neural networks, \verb/AI::FANN::Evolving/ (\url{https://github.com/naturalis/ai-fann-evolving}), was implemented. The library was implemented the training script to integrate the genetic algorithm in hierarchical neural network training.
